...
inline std::string RangingDoneStatusText(const RangingDoneStatus& param) {switch (param) {case RangingDoneStatus::ALL_RESULTS_COMPLETE: return "ALL_RESULTS_COMPLETE(0x0)";case RangingDoneStatus::PARTIAL_RESULTS: return "PARTIAL_RESULTS(0x1)";case RangingDoneStatus::ABORTED: return "ABORTED(0xf)";default: return std::format("Unknown RangingDoneStatus({:#02x})", static_cast<uint64_t>(param));}}



inline std::string SubeventDoneStatusText(const SubeventDoneStatus& param) {switch (param) {case SubeventDoneStatus::ALL_RESULTS_COMPLETE: return "ALL_RESULTS_COMPLETE(0x0)";case SubeventDoneStatus::ABORTED: return "ABORTED(0xf)";default: return std::format("Unknown SubeventDoneStatus({:#02x})", static_cast<uint64_t>(param));}}



inline std::string RangingAbortReasonText(const RangingAbortReason& param) {switch (param) {case RangingAbortReason::NO_ABORT: return "NO_ABORT(0x0)";case RangingAbortReason::LOCAL_HOST_OR_REMOTE: return "LOCAL_HOST_OR_REMOTE(0x1)";case RangingAbortReason::INSUFFICIENT_FILTERED_CHANNELS: return "INSUFFICIENT_FILTERED_CHANNELS(0x2)";case RangingAbortReason::INSTANT_HAS_PASSED: return "INSTANT_HAS_PASSED(0x3)";case RangingAbortReason::UNSPECIFIED: return "UNSPECIFIED(0xf)";default: return std::format("Unknown RangingAbortReason({:#02x})", static_cast<uint64_t>(param));}}



inline std::string SubeventAbortReasonText(const SubeventAbortReason& param) {switch (param) {case SubeventAbortReason::NO_ABORT: return "NO_ABORT(0x0)";case SubeventAbortReason::LOCAL_HOST_OR_REMOTE: return "LOCAL_HOST_OR_REMOTE(0x1)";case SubeventAbortReason::NO_CS_SYNC_RECEIVED: return "NO_CS_SYNC_RECEIVED(0x2)";case SubeventAbortReason::SCHEDULING_CONFLICTS_OR_LIMITED_RESOURCES: return "SCHEDULING_CONFLICTS_OR_LIMITED_RESOURCES(0x3)";case SubeventAbortReason::UNSPECIFIED: return "UNSPECIFIED(0xf)";default: return std::format("Unknown SubeventAbortReason({:#02x})", static_cast<uint64_t>(param));}}
...
static Iterator<kLittleEndian> Parse(StepMode* to_fill, Iterator<kLittleEndian> struct_begin_it ) {auto to_bound = struct_begin_it;size_t end_index = struct_begin_it.NumBytesRemaining();if (end_index < 1){ return struct_begin_it.Subrange(0,0);}{if (to_bound.NumBytesRemaining() < 1){ return to_bound.Subrange(to_bound.NumBytesRemaining(),0);}}{auto mode_type_it = to_bound + 0;auto mode_type_ptr = &to_fill->mode_type_;auto extracted_value = mode_type_it.extract<uint8_t>();extracted_value &= 0x3;*mode_type_ptr = static_cast<uint8_t>(extracted_value);}{auto aborted_it = to_bound + 0;auto aborted_ptr = &to_fill->aborted_;auto extracted_value = aborted_it.extract<uint8_t>();extracted_value >>= 7;extracted_value &= 0x1;*aborted_ptr = static_cast<uint8_t>(extracted_value);}return struct_begin_it + to_fill->size();}
protected:public:virtual size_t size() const override {return 1;}


std::string ToString() const {std::stringstream ss;ss << std::hex << std::showbase << "StepMode { ";ss << "mode_type = " << +mode_type_ << ", aborted = " << +aborted_;ss << " }";return ss.str();}
...
static Iterator<kLittleEndian> Parse(RasSubeventHeader* to_fill, Iterator<kLittleEndian> struct_begin_it ) {auto to_bound = struct_begin_it;size_t end_index = struct_begin_it.NumBytesRemaining();if (end_index < 8){ return struct_begin_it.Subrange(0,0);}{if (to_bound.NumBytesRemaining() < 8){ return to_bound.Subrange(to_bound.NumBytesRemaining(),0);}}{auto start_acl_conn_event_it = to_bound + 0;auto start_acl_conn_event_ptr = &to_fill->start_acl_conn_event_;auto extracted_value = start_acl_conn_event_it.extract<uint16_t>(2);*start_acl_conn_event_ptr = static_cast<uint16_t>(extracted_value);}{auto frequency_compensation_it = to_bound + 2;auto frequency_compensation_ptr = &to_fill->frequency_compensation_;auto extracted_value = frequency_compensation_it.extract<uint16_t>(2);*frequency_compensation_ptr = static_cast<uint16_t>(extracted_value);}{auto ranging_done_status_it = to_bound + 4;auto ranging_done_status_ptr = &to_fill->ranging_done_status_;auto extracted_value = ranging_done_status_it.extract<uint8_t>();extracted_value &= 0xf;*ranging_done_status_ptr = static_cast<RangingDoneStatus>(extracted_value);}{auto subevent_done_status_it = to_bound + 4;auto subevent_done_status_ptr = &to_fill->subevent_done_status_;auto extracted_value = subevent_done_status_it.extract<uint8_t>();extracted_value >>= 4;extracted_value &= 0xf;*subevent_done_status_ptr = static_cast<SubeventDoneStatus>(extracted_value);}{auto ranging_abort_reason_it = to_bound + 5;auto ranging_abort_reason_ptr = &to_fill->ranging_abort_reason_;auto extracted_value = ranging_abort_reason_it.extract<uint8_t>();extracted_value &= 0xf;*ranging_abort_reason_ptr = static_cast<RangingAbortReason>(extracted_value);}{auto subevent_abort_reason_it = to_bound + 5;auto subevent_abort_reason_ptr = &to_fill->subevent_abort_reason_;auto extracted_value = subevent_abort_reason_it.extract<uint8_t>();extracted_value >>= 4;extracted_value &= 0xf;*subevent_abort_reason_ptr = static_cast<SubeventAbortReason>(extracted_value);}{auto reference_power_level_it = to_bound + 6;auto reference_power_level_ptr = &to_fill->reference_power_level_;auto extracted_value = reference_power_level_it.extract<uint8_t>(1);*reference_power_level_ptr = static_cast<uint8_t>(extracted_value);}{auto num_steps_reported_it = to_bound + 7;auto num_steps_reported_ptr = &to_fill->num_steps_reported_;auto extracted_value = num_steps_reported_it.extract<uint8_t>(1);*num_steps_reported_ptr = static_cast<uint8_t>(extracted_value);}return struct_begin_it + to_fill->size();}
protected:public:virtual size_t size() const override {return 8;}


std::string ToString() const {std::stringstream ss;ss << std::hex << std::showbase << "RasSubeventHeader { ";ss << "start_acl_conn_event = " << start_acl_conn_event_ << ", frequency_compensation = " << frequency_compensation_ << ", ranging_done_status = " << RangingDoneStatusText(ranging_done_status_) << ", subevent_done_status = " << SubeventDoneStatusText(subevent_done_status_) << ", ranging_abort_reason = " << RangingAbortReasonText(ranging_abort_reason_) << ", subevent_abort_reason = " << SubeventAbortReasonText(subevent_abort_reason_) << ", reference_power_level = " << +reference_power_level_ << ", num_steps_reported = " << +num_steps_reported_;ss << " }";return ss.str();}
...
RangingHeader(uint16_t ranging_counter, uint8_t configuration_id, uint8_t selected_tx_power, uint8_t antenna_paths_mask)
 : ranging_counter_(ranging_counter), configuration_id_(configuration_id), selected_tx_power_(selected_tx_power), antenna_paths_mask_(antenna_paths_mask)
{}
 public:
  virtual ~RangingHeader() = default;
protected:void SerializeHeader(BitInserter& i ) const {insert(ranging_counter_, i,12);insert(configuration_id_, i,4);i.insert_byte(selected_tx_power_);insert(antenna_paths_mask_, i,4);insert(static_cast<uint8_t>(0) /* Reserved */, i, 4 );
}
...
static Iterator<kLittleEndian> Parse(RangingHeader* to_fill, Iterator<kLittleEndian> struct_begin_it ) {auto to_bound = struct_begin_it;size_t end_index = struct_begin_it.NumBytesRemaining();if (end_index < 4){ return struct_begin_it.Subrange(0,0);}{if (to_bound.NumBytesRemaining() < 4){ return to_bound.Subrange(to_bound.NumBytesRemaining(),0);}}{auto ranging_counter_it = to_bound + 0;auto ranging_counter_ptr = &to_fill->ranging_counter_;auto extracted_value = ranging_counter_it.extract<uint16_t>();extracted_value &= 0xfff;*ranging_counter_ptr = static_cast<uint16_t>(extracted_value);}{auto configuration_id_it = to_bound + 1;auto configuration_id_ptr = &to_fill->configuration_id_;auto extracted_value = configuration_id_it.extract<uint8_t>();extracted_value >>= 4;extracted_value &= 0xf;*configuration_id_ptr = static_cast<uint8_t>(extracted_value);}{auto selected_tx_power_it = to_bound + 2;auto selected_tx_power_ptr = &to_fill->selected_tx_power_;auto extracted_value = selected_tx_power_it.extract<uint8_t>(1);*selected_tx_power_ptr = static_cast<uint8_t>(extracted_value);}{auto antenna_paths_mask_it = to_bound + 3;auto antenna_paths_mask_ptr = &to_fill->antenna_paths_mask_;auto extracted_value = antenna_paths_mask_it.extract<uint8_t>();extracted_value &= 0xf;*antenna_paths_mask_ptr = static_cast<uint8_t>(extracted_value);}return struct_begin_it + to_fill->size();}
protected:public:virtual size_t size() const override {return 4;}


std::string ToString() const {std::stringstream ss;ss << std::hex << std::showbase << "RangingHeader { ";ss << "ranging_counter = " << ranging_counter_ << ", configuration_id = " << +configuration_id_ << ", selected_tx_power = " << +selected_tx_power_ << ", antenna_paths_mask = " << +antenna_paths_mask_;ss << " }";return ss.str();}

uint16_t ranging_counter_{};uint8_t configuration_id_{};uint8_t selected_tx_power_{};uint8_t antenna_paths_mask_{};};
...
static Iterator<kLittleEndian> Parse(SegmentationHeader* to_fill, Iterator<kLittleEndian> struct_begin_it ) {auto to_bound = struct_begin_it;size_t end_index = struct_begin_it.NumBytesRemaining();if (end_index < 1){ return struct_begin_it.Subrange(0,0);}{if (to_bound.NumBytesRemaining() < 1){ return to_bound.Subrange(to_bound.NumBytesRemaining(),0);}}{auto first_segment_it = to_bound + 0;auto first_segment_ptr = &to_fill->first_segment_;auto extracted_value = first_segment_it.extract<uint8_t>();extracted_value &= 0x1;*first_segment_ptr = static_cast<uint8_t>(extracted_value);}{auto last_segment_it = to_bound + 0;auto last_segment_ptr = &to_fill->last_segment_;auto extracted_value = last_segment_it.extract<uint8_t>();extracted_value >>= 1;extracted_value &= 0x1;*last_segment_ptr = static_cast<uint8_t>(extracted_value);}{auto rolling_segment_counter_it = to_bound + 0;auto rolling_segment_counter_ptr = &to_fill->rolling_segment_counter_;auto extracted_value = rolling_segment_counter_it.extract<uint8_t>();extracted_value >>= 2;extracted_value &= 0x3f;*rolling_segment_counter_ptr = static_cast<uint8_t>(extracted_value);}return struct_begin_it + to_fill->size();}
protected:public:virtual size_t size() const override {return 1;}


std::string ToString() const {std::stringstream ss;ss << std::hex << std::showbase << "SegmentationHeader { ";ss << "first_segment = " << +first_segment_ << ", last_segment = " << +last_segment_ << ", rolling_segment_counter = " << +rolling_segment_counter_;ss << " }";return ss.str();}
...
#endif
RasSubeventHeader GetSubeventHeader() const {ASSERT(was_validated_);size_t end_index = size();auto to_bound = begin();size_t field_begin = (0) / 8;size_t field_end = field_begin + (64) / 8;if (field_end > end_index) { field_end = end_index; }auto subevent_header_it = to_bound.Subrange(field_begin, field_end - field_begin); RasSubeventHeader subevent_header_value{};RasSubeventHeader* subevent_header_ptr = &subevent_header_value;subevent_header_it = RasSubeventHeader::Parse(subevent_header_ptr, subevent_header_it);return subevent_header_value;}

std::vector<uint8_t> GetSubeventData() const {ASSERT(was_validated_);size_t end_index = size();auto to_bound = begin();size_t field_begin = (64) / 8;size_t field_end = end_index - (0) / 8;auto subevent_data_it = to_bound.Subrange(field_begin, field_end - field_begin); std::vector<uint8_t> subevent_data_value{};std::vector<uint8_t>* subevent_data_ptr = &subevent_data_value;auto val_it = subevent_data_it;while (val_it.NumBytesRemaining() >= 1) {uint8_t val_value;uint8_t* val_ptr = &val_value;auto extracted_value = val_it.extract<uint8_t>(1);*val_ptr = static_cast<uint8_t>(extracted_value);if (val_ptr != nullptr) { subevent_data_ptr->push_back(val_value);}}return subevent_data_value;}
...
auto it = begin() + (0) / 8;it += 8 /* Total size of the fixed fields */;if (it > end()) return false;

return true;}
bool was_validated_{false};

 public:virtual std::string ToString() const  {std::stringstream ss;ss << std::showbase << std::hex << "RasSubevent { ";ss << ""  << "subevent_header = " << GetSubeventHeader().ToString() << ", subevent_data = " << "VECTOR[";for (size_t index = 0; index < GetSubeventData().size(); index++) {ss << ((index == 0) ? "" : ", ") << +(GetSubeventData()[index]);}ss << "]";ss << " }";return ss.str();}
...
#endif
SegmentationHeader GetSegmentationHeader() const {ASSERT(was_validated_);size_t end_index = size();auto to_bound = begin();size_t field_begin = (0) / 8;size_t field_end = field_begin + (8) / 8;if (field_end > end_index) { field_end = end_index; }auto segmentation_header_it = to_bound.Subrange(field_begin, field_end - field_begin); SegmentationHeader segmentation_header_value{};SegmentationHeader* segmentation_header_ptr = &segmentation_header_value;segmentation_header_it = SegmentationHeader::Parse(segmentation_header_ptr, segmentation_header_it);return segmentation_header_value;}

RangingHeader GetRangingHeader() const {ASSERT(was_validated_);size_t end_index = size();auto to_bound = begin();size_t field_begin = (8) / 8;size_t field_end = field_begin + (32) / 8;if (field_end > end_index) { field_end = end_index; }auto ranging_header_it = to_bound.Subrange(field_begin, field_end - field_begin); RangingHeader ranging_header_value{};RangingHeader* ranging_header_ptr = &ranging_header_value;ranging_header_it = RangingHeader::Parse(ranging_header_ptr, ranging_header_it);return ranging_header_value;}

std::vector<uint8_t> GetData() const {ASSERT(was_validated_);size_t end_index = size();auto to_bound = begin();size_t field_begin = (40) / 8;size_t field_end = end_index - (0) / 8;auto data_it = to_bound.Subrange(field_begin, field_end - field_begin); std::vector<uint8_t> data_value{};std::vector<uint8_t>* data_ptr = &data_value;auto val_it = data_it;while (val_it.NumBytesRemaining() >= 1) {uint8_t val_value;uint8_t* val_ptr = &val_value;auto extracted_value = val_it.extract<uint8_t>(1);*val_ptr = static_cast<uint8_t>(extracted_value);if (val_ptr != nullptr) { data_ptr->push_back(val_value);}}return data_value;}
...
protected:
virtual bool Validate() const {
auto it = begin() + (0) / 8;it += 5 /* Total size of the fixed fields */;if (it > end()) return false;


return true;}
bool was_validated_{false};

 public:virtual std::string ToString() const  {std::stringstream ss;ss << std::showbase << std::hex << "FirstRangingDataSegment { ";ss << ""  << "segmentation_header = " << GetSegmentationHeader().ToString() << ", ranging_header = " << GetRangingHeader().ToString() << ", data = " << "VECTOR[";for (size_t index = 0; index < GetData().size(); index++) {ss << ((index == 0) ? "" : ", ") << +(GetData()[index]);}ss << "]";ss << " }";return ss.str();}
...
std::vector<uint8_t> GetData() const {ASSERT(was_validated_);size_t end_index = size();auto to_bound = begin();size_t field_begin = (8) / 8;size_t field_end = end_index - (0) / 8;auto data_it = to_bound.Subrange(field_begin, field_end - field_begin); std::vector<uint8_t> data_value{};std::vector<uint8_t>* data_ptr = &data_value;auto val_it = data_it;while (val_it.NumBytesRemaining() >= 1) {uint8_t val_value;uint8_t* val_ptr = &val_value;auto extracted_value = val_it.extract<uint8_t>(1);*val_ptr = static_cast<uint8_t>(extracted_value);if (val_ptr != nullptr) { data_ptr->push_back(val_value);}}return data_value;}
...
protected:
virtual bool Validate() const {
auto it = begin() + (0) / 8;it += 1 /* Total size of the fixed fields */;if (it > end()) return false;

return true;}
bool was_validated_{false};

 public:virtual std::string ToString() const  {std::stringstream ss;ss << std::showbase << std::hex << "RangingDataSegment { ";ss << ""  << "segmentation_header = " << GetSegmentationHeader().ToString() << ", data = " << "VECTOR[";for (size_t index = 0; index < GetData().size(); index++) {ss << ((index == 0) ? "" : ", ") << +(GetData()[index]);}ss << "]";ss << " }";return ss.str();}
...
protected:size_t BitsOfHeaderAndFooter() const {return 0 + /* Bits: */ 0 + /* Dynamic: */ (subevent_header_.size() * 8) + /* Bits: */ 0 + /* Dynamic: */ (static_cast<size_t>(subevent_data_.size()) * 8);}

public:virtual size_t size() const override {return (BitsOfHeaderAndFooter() / 8);}
 protected:
...
protected:size_t BitsOfHeaderAndFooter() const {return 0 + /* Bits: */ 0 + /* Dynamic: */ (segmentation_header_.size() * 8) + /* Bits: */ 0 + /* Dynamic: */ (ranging_header_.size() * 8) + /* Bits: */ 0 + /* Dynamic: */ (static_cast<size_t>(data_.size()) * 8);}
...
protected:size_t BitsOfHeaderAndFooter() const {return 0 + /* Bits: */ 0 + /* Dynamic: */ (segmentation_header_.size() * 8) + /* Bits: */ 0 + /* Dynamic: */ (static_cast<size_t>(data_.size()) * 8);}
